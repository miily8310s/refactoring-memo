...
- 追加しようとしてる機能が非常に些細な場合は、昨日の機能だけをおこなってリファクタリングを後回しにすることもある
- 新機能の実装が楽になると感じるときは、リファクタリングをまず行う
- 最も危険なのは、「美しいコード」「すばらしいエンジニアリングのプラクティス」といった道徳的理由により、リファクタリングが正当化される状況
- リファクタリングするのは、あくまでも新機能の追加やバグの修正を速めるため

### コードの所有権

- 別のチームが担当しており、そのリポジトリに書き込み権限がない状況では単純なリファクタリングさえ出来ない
- コードの所有者による境界は、リファクタリングにとって妨げになる
- そのため、強いコードの所有権を細かく設定するするべきではない
- 寛容な所有権のルールとして、他のチームの変更は、分岐したブランチのコミットとして出し、担当チームに受け入れてもうらうようにする方法がある

### ブランチ

- 機能ごとにブランチを切り、作業してからリリースが可能になったあとで`master`などのメインブランチにマージするやり方が一般的
- このような機能ごとのブランチ方式には欠点がある
- 分離したブランチでの作業が長くなるほど、作業後にメインブランチに統合するのが難しくなる
  - たとえば他の人が書いた結果がメインブランチにマージされたとする
  - 自分が作業している間は、その他の人の変更内容を知ることができない
  - つまり双方が影響を受ける可能性がある
- 継続的インテグレーション（CI）とリファクタリングを協調するとよい
  - 常にメインブランチが健全に保たれるよう、大きな機能を細かな機能に分割するなどの、プラクティスの実施は必要

### テスト

- リファクタリングの手順に注意深く従って進めば、プログラムが壊れることはない
- ひとつひとつのリファクタリングは小さな変更なので、何かを壊したとしても誤りを探す範囲は限定される
- リファクタリングをしたいなら、ほとんどの場合、自己テストコードが必要

### レガシーコード

- レガシーなコードは大抵の場合は複雑で、テストもろく書かれておらず、何よりも「他の誰か」によって書かれている
- もしもテストが存在しない巨大なレガシーシステムが相手なら、リファクタリングして明確にすることは安全には出来ない
- この問題の答えは、テスト追加していくこと
- 悲しいことに解決に至る近道はない
- 「**レガシーコード改善ガイド**」に書かれているガイダンスに従うのが一番
- 結局は最初から自己テストコードを書いていくことが強く推奨される

### データベース

- かつてデータベースのリファクタリングは問題の多い分野だった
- しかし現在は、データベーススキーマやデータアクセスのコードの構造的な変更に、データ移行のためのスクリプトを簡単に構成できるように組み合わせる手法が確立された
- 通常のリファクタリングとの違いは、データベースの変更は、正式に至るまでにふくすのリリースに分割するのがベスト

## リファクタリングとアーキテクチャ、そして Yagni

- リファクタリングはソフトウェアアーキテクチャの考えるを根本から変えたもの
- その前までは、ソフトウェア設計は、コードを書き始める前に入念に取り組み、ほぼ完成しているものとされていた
  - ただしこの手法の最大の問題点は、ソフトウェアの要求は事前に十分把握できるものという前提に立っていること
  - 実際は、ソフトウェアを使ってみて初めて、本当に求めることが判明した例がいくつもある
  - ビジネスに支障をきたすケースもあった
- リファクタリングが与えた最も大きい影響は、要求の変化にしなやかに対応できる、すぐれた設計のコードベースを作り上げる方法を示したこと
  - まずは現在わかっている要求のみを解決するソフトウェアを構築する
  - ユーザ要求の変化に合わせて、アーキテクチャを新たな要求に適合させる
- 後でリファクタリングするほうが難しいとかんじられるときには、柔軟性の仕組みの導入を初めて考える
  - 柔軟性の仕組みは、変化に対応する速度を「遅く」させるため注意が必要
- この設計手法は**Yagni**などと呼ばれる
  - この手法はリファクタリングなしでは実現できない

## リファクタリングとソフトウェア開発プロセス

- エクストリーム・プログラミング（以下 XP）は、リファクタリングを早期からプラクティスとして組み込んでいた
- XP は、プラクティスを組み合わせる手法
  - 例）継続的インテグレーション、自己テストコード、リファクタリングなど
- アジャイルプロジェクトでは、真にアジャイルなやり方を実践するために、チームメンバーはリファクタリングに熱心に取り組むべき
  - 開発プロセスの多くは、リファクタリングが通常の作業の一部になるように合わせる必要がある
- **Yagni**という設計手法は、継続的インテグレーション、自己テストコード、リファクタリングの三つのプラクティスの実践があって初めて可能になる
  - プラクティスのバランスを取ることで、変化する要求に即座に答えられ、かつ信頼性も高いコードベースを実現できる
- ただし、上記手法がうまくいくには実践とスキルは必要

## リファクタリングの起源

MEMO: 手法から外れる話のため、一旦飛ばす
