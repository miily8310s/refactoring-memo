...
- コレクションのパイプラインを使用すると、コレクションを消費したり出力したりする各処理を一連の操作として記述できる
  - 最も一般的なものは、map と filter
- パイプライン構造で表現すると、ロジックを追うのがはるかに簡単になる

### 手順

1. ループ処理のコレクション用に新しい変数を作成する
2. ループ内の各処理を上から一つずつ取り出し、コレクションのパイプライン操作に置き換えて、ループ処理のコレクション用変数を加工するパイプラインに付け加える。変更のたびにテストする
3. すべての処理をループから削除できたら、ループを取り除く
...
- コードが使用されなくなったら削除すべき。そのコードが将来必要になるかもしれないなどの心配はいらない
